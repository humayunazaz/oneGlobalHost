...
export class SingleInstancesComponent implements OnInit {
  ...
  gotoDetail(event){
    if(this.path.indexOf('general') > -1){
      this.router.navigate(['infrastructure/general/singleinstance', event.id]);
    } else if(this.path.indexOf('powerConfigs') > -1){
      this.router.navigate(['infrastructure/powerConfigs/singleinstance', event.id]);
    } else{
      this.router.navigate(['infrastructure/largeStorage/singleinstance', event.id]);
    }
