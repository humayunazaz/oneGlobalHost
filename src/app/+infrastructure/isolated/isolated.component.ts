...
export class IsolatedComponent implements OnInit {
  ...
  gotoDetail(event){
    if(this.path.indexOf('general') > -1){
      this.router.navigate(['infrastructure/general/isolated', event.id]);
    } else if(this.path.indexOf('powerConfigs') > -1){
      this.router.navigate(['infrastructure/powerConfigs/isolated', event.id]);
    } else{
      this.router.navigate(['infrastructure/largeStorage/isolated', event.id]);
    }
  }
  ngOnInit() {
