...
export class InternetAccessComponent implements OnInit {
  ...
  gotoDetail(event){
    if(this.path.indexOf('general') > -1){
      this.router.navigate(['infrastructure/general/internetaccess', event.id]);
    } else if(this.path.indexOf('powerConfigs') > -1){
      this.router.navigate(['infrastructure/powerConfigs/internetaccess', event.id]);
    } else{
      this.router.navigate(['infrastructure/largeStorage/internetaccess', event.id]);
    }
